...
import { fetchGetAllTasks } from "@/components/assign_tasks/api/fetch-get-all-tasks"
import AssignTasksTable from "@/components/assign_tasks/components/AssignTasksTable"
...
function AssignTasks() {
  ...
  const [taskPriority, setTaskPriority] = useState<"C" | "H" | "M" | "L" | "" | string>("C")
  ...
  const handleDeleteTask = async (taskid: number | string, setOpenDeletedConfirmModel: any) => {
  ...
  const handleUpdateTask = async (task: any, setSelectedTask: any, setOpenEditModel: any) => {
    ...
    <section className="w-full bg-white rounded-lg p-6">
      <div className="w-full flex flex-col justify-between gap-4">
        <div className="w-full flex items-center justify-between">
          <h1 className="text-xl font-medium">Assign Tasks</h1>
          <Button className="bg-[#141D38] hover:bg-[#0f162e] text-white font-medium py-2 px-4 rounded-sm shadow-md transition-all disabled:opacity-50 cursor-pointer" onClick={() => { setOpenTask(true); handleFetchEmployees() }}>Assign Task</Button>
        </div>
        <AssignTasksTable tasksData={tasksData} handleDeleteTask={handleDeleteTask} handleUpdateTask={handleUpdateTask} />
        <Dialog onOpenChange={setOpenTask} open={openTask}>
          <DialogContent>
            ...
            <form action="" className="flex flex-col gap-4" onSubmit={handleCreateTask}>
              ...
              <Button className="bg-[#141D38] hover:bg-[#0f162e] text-white font-medium py-2 px-4 rounded-sm shadow-md transition-all disabled:opacity-50 cursor-pointer" type="submit">Assign Task</Button>
            </form>
          </DialogContent>
        </Dialog>
      </div>
